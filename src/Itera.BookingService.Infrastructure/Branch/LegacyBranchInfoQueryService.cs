...
public sealed class LegacyBranchInfoQueryService(LegacyDbContext dbContext) : IBranchInfoQueryService
{
    ...
    public async Task<WsFiliale?> GetInfoBranchAsync(short brandId, int branchId, bool getFilialiExtra, byte languageId, DateTime selectedDate, CancellationToken cancellationToken)
    {
        ...
        return MapToWsFiliale(row, translations, includeExtraData: true, languageId, closingExtras, timeSlots, operativeSlots, restDays);
    }
    ...
    private static WsFiliale MapToWsFiliale(
        ...
        byte languageId,
        IReadOnlyList<FilialeChiusuraExtra> closingExtras,
        IReadOnlyList<FilialeFasciaOrario> timeSlots,
        IReadOnlyList<FilialeOrarioOperativo> operativeSlots,
        IReadOnlyList<FilialeRiposoSettimanale> restDays)
    {
